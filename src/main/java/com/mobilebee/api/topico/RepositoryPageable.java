...
public interface RepositoryPageable extends PagingAndSortingRepository<Topico, Long>{
    ...
    "t.status as status, t.autor as autor, t.curso as curso from Topico as t where status = true")
    Page<DtoRead> listar(Pageable p);

    @Query("select t.titulo as titulo, t.mensagem as mensagem, t.dataCriacao as dataCriacao, " +
    "t.status as status, t.autor as autor, t.curso as curso from Topico as t where year(t.dataCriacao) = :ano and status = true")
